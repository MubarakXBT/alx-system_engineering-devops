...
if __name__ == "__main__":
    ...
    for task in file:
        if task["completed"] is True:
            dic = {'USER_ID': ID, 'USERNAME': username,'TASK_COMPLETED_STATUS': True, 'TASK_TITLE': task["title"]}
            data.append(dic)
        else:
            dic = {'USER_ID': ID, 'USERNAME': username,'TASK_COMPLETED_STATUS': False, 'TASK_TITLE': task["title"]}
            data.append(dic)
